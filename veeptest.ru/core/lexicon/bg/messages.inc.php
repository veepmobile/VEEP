<?php
/**
 * Messages English lexicon topic
 *
 * @language en
 * @package modx
 * @subpackage lexicon
 */
$_lang['all'] = 'Всички';
$_lang['date_sent'] = 'Данни изпратени';
$_lang['forward'] = 'Препрати';
$_lang['mark_unread'] = 'Маркирай непрочетените';
$_lang['message'] = 'Съобщение';
$_lang['message_create'] = 'Създай съобщение';
$_lang['message_new'] = 'Ново съобщение';
$_lang['message_remove_confirm'] = 'Сигурен ли сте, че искате да премахнете това съобщение напълно?';
$_lang['messages'] = 'Съобщения';
$_lang['private'] = 'Лично';
$_lang['read'] = 'Прочети';
$_lang['recipient'] = 'Получател';
$_lang['recipient_type'] = 'Тип получател';
$_lang['reply'] = 'Отговори';
$_lang['role'] = 'Роля';
$_lang['sender'] = 'Подател';
$_lang['sent_by'] = 'Изпратено от';
$_lang['sent_on'] = 'Изпратено на';
$_lang['subject'] = 'Тема';
$_lang['user'] = 'Потребител';
$_lang['usergroup'] = 'Потребителска група';

$_lang['message_err_nfs'] = 'Съобщението не е намерено.';
$_lang['message_err_not_specified_subject'] = 'Не е посочена тема!';
$_lang['message_err_remove'] = 'Възникна грешка в опита за изтриване на съобщението.';
$_lang['message_err_remove_notauth'] ='Не сте упълномощен да изтриете това съобщение.';
$_lang['message_err_save'] = 'Възникна грешка при записа на съобщението.';
$_lang['message_send_email'] = 'Send Email';
$_lang['messages_all'] = 'Всички';
$_lang['messages_compose'] = 'Създай съобщение';
$_lang['messages_forward'] = 'Препрати';
$_lang['messages_from'] = 'От';
$_lang['messages_group'] = 'Група';
$_lang['messages_inbox'] = 'Входяща кутия';
$_lang['messages_mark_as_unread'] = 'Маркирай като непрочетено';
$_lang['messages_message'] = 'Съобщение';
$_lang['messages_not_allowed_to_read'] = 'Не ви е позволено да четете това съобщение!';
$_lang['messages_no_messages'] = 'Няма съобщения в папка "Входящи".';
$_lang['messages_private'] = 'Лично';
$_lang['messages_read_message'] = 'Прочети съобщението';
$_lang['messages_reply'] = 'Отговори';
$_lang['messages_role'] = 'Роля';
$_lang['messages_select_group'] = 'Изберете група';
$_lang['messages_select_user'] = 'Изберете потребител';
$_lang['messages_send'] = 'Изпрати';
$_lang['messages_send_to'] = 'Изпрати до';
$_lang['messages_sent'] = 'Изпратено на';
$_lang['messages_subject'] = 'Тема';
$_lang['messages_system_user'] = '[System]';
$_lang['messages_title'] = 'Съобщения';
$_lang['messages_user'] = 'Потребител';