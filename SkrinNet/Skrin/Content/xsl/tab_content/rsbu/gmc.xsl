	...
	<xsl:template match="profile">
		<xsl:if test="//@PDF=-1">
            ...
            <div class="content_menu">
              ...
              <div class="content_menu_sel" style="cursor:pointer;">
							<xsl:attribute name="onclick">
								goform(2,'<xsl:value-of select="//@iss"/>')
							</xsl:attribute>
							Отчет об изменениях капитала (Форма № 3)<br/>Отчет  о движении денежных средств (Форма №4)<br/>Приложение к бухгалтерскому балансу (Форма № 5)<xsl:if test="//type_id/@val=7"><br/>Отчет о целевом использовании средств(Форма №6)</xsl:if>
						</div>
		...
		<form id="frm">
			<table id="curr_selector" onclick="checkchecked(event);" style="border:none;background-color:#f6f7fa ;">
				<tr>
					...
					<td style="border:none;background-color:#f6f7fa ;"><input type="text" id="inpselector" readonly="readonly" class="system_form" onclick="showchbselector(event)" style="width:180px;"><xsl:attribute name="value"><xsl:for-each select="per"><xsl:if test="@sel=1"><xsl:value-of select="@yq"/>,</xsl:if></xsl:for-each></xsl:attribute></input>
