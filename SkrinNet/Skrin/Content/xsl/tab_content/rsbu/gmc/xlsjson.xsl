		...
		<xsl:if test="f">
			<xsl:for-each select="f">
					<xsl:for-each select="s">
							{WorkSheet:"ФОРМА № <xsl:value-of select="../@form_no"/><xsl:if test="@id &gt;0"> (<xsl:value-of select="@id"/>)</xsl:if> ГМЦ",
							"Cols":[310,80,80,<xsl:for-each select="fa[1]/Form[1]/c">120<xsl:if test="position()!=last()">,</xsl:if></xsl:for-each>],"IssName": "<xsl:value-of disable-output-escaping="yes" select="//name/@val"/>", "Style": "Header", "Header" : "Бухгалтерская отчетность по РСБУ по данным ГМЦ Росстата за <xsl:for-each select="//per"><xsl:if test="@sel=1"><xsl:value-of select="@yq"/> г.</xsl:if></xsl:for-each>, <xsl:for-each select="//currency">
								<xsl:if test="@sel=1">
									<xsl:value-of select ="@name"/>
								</xsl:if>
							</xsl:for-each>",
							"SheetData" : [
							{Row :[{"Style" : "subcaption","colspan":<xsl:value-of select="count(fa[1]/Form[1]/c)+3"/>, "rowspan":1, "Type" : "S", "Data" : "<xsl:if test="position()>1"><xsl:value-of select="js:STransform(string(@name))"/></xsl:if><xsl:if test="position()=1"><xsl:value-of select="@name"/>(ФОРМА № <xsl:value-of select="../@form_no"/>)</xsl:if>"},]},
							<xsl:for-each select="fa">
						<xsl:if test="position()=1">
							{Row :[{"Style" : "TabHeader","colspan":2, "rowspan":1, Type:"S", "Data" : "Наименование показателя"},
            ...
            <xsl:if test="(//cs/@val &gt; 1  and position()=1)">
					      {Row :[{"Style" : "even","colspan":2, "rowspan":1, Type:"S", "Data" : "Курс <xsl:value-of select="//kurs_header/@val"/>"},{"Style" : "even","colspan":1, "rowspan":1,  Type:"S", "Data" : ""},<xsl:for-each select="Form[1]/c">
						    {"Style" : "even_right8","colspan":1, "rowspan":1, Type:"N", "Data" : <xsl:value-of select="v/@kurs"/>},
					      </xsl:for-each>]},
					    {Row :[{"Style" : "even","colspan":2, "rowspan":1, Type:"S", "Data" : "Курс на дату"},{"Style" : "even","colspan":1, "rowspan":1,  Type:"S", "Data" : ""},<xsl:for-each select="Form[1]/c">
						    {"Style" : "even_date","colspan":1, "rowspan":1, Type:"D", "Data" : "<xsl:value-of select="v/@cd"/>"},
					  </xsl:for-each>]},
				      </xsl:if>
						<xsl:if test="(../../@form_no=1 and position()=1)">
            
							{Row :[{"Style" : "bold","colspan":2, "rowspan":1, Type:"S", "Data" : "Актив"},{"Style" : "even","colspan":1, "rowspan":1,  Type:"S", "Data" : ""},<xsl:for-each select="Form[1]/c">{"Style" : "bold","colspan":1, "rowspan":1, Type:"N", "Data" : ""},</xsl:for-each>]},
						</xsl:if>
						<xsl:if test="string-length(@name) &gt; 0 and not(//type_id/@val=7 or //type_id/@val=8)">
							{Row :[{"Style" : "bold","colspan":2, "rowspan":1, Type:"S", "Data" : "<xsl:value-of select="@name"/>"},{"Style" : "even","colspan":1, "rowspan":1,  Type:"S", "Data" : ""},<xsl:for-each select="Form[1]/c">{"Style" : "bold","colspan":1, "rowspan":1, Type:"N", "Data" : ""},</xsl:for-each>]},
						</xsl:if>
						<xsl:for-each select="Form">
							{Row :[<xsl:if  test="@rowspan &gt; 0 ">
								{"Style" : "<xsl:choose>
									<xsl:when test="ceiling(position() div 2)- position() div 2 = 0">even</xsl:when>
									<xsl:otherwise>even</xsl:otherwise>
								</xsl:choose>","colspan":2, "rowspan":<xsl:choose>
								<xsl:when test="@rowspan &gt; 0 and //type_id/@val=7">
									<xsl:value-of select="@rowspan"/>
								</xsl:when>
								<xsl:otherwise>1</xsl:otherwise></xsl:choose>, "Type" : "S", "Data" : "<xsl:value-of disable-output-escaping ="yes" select="@name"/>"},
							</xsl:if>
							{"Style" : "<xsl:choose>
								<xsl:when test="ceiling(position() div 2)- position() div 2 = 0">even</xsl:when>
								<xsl:otherwise>even</xsl:otherwise>
							</xsl:choose>","colspan":1, "rowspan":1,  "Type" : "S",<xsl:choose><xsl:when test="@rowspan=0">"Index":2,</xsl:when><xsl:otherwise>"Index":0,</xsl:otherwise></xsl:choose>"Data" : "<xsl:value-of disable-output-escaping ="yes" select="@line_code"/>"},
							<xsl:call-template name="num_nodes">
								<xsl:with-param name="odd">
									<xsl:choose>
										<xsl:when test="ceiling(position() div 2)- position() div 2 = 0">even_right</xsl:when>
							...
							<xsl:if test="//f/@form_no=1">
								<xsl:if test="@line_code=300 or @line_code=1600">
									{Row :[{"Style" : "bold","colspan":2, "rowspan":1, Type:"S", "Data" : "Пассив"},{"Style" : "even","colspan":1, "rowspan":1, "Index": 1, Type:"S", "Data" : ""},<xsl:for-each select="c">{"Style" : "bold","colspan":1, "rowspan":1, Type:"N", "Data" : ""},</xsl:for-each>]},
								</xsl:if>
							</xsl:if>
		...
		<xsl:if test="headers">
			...
			<xsl:for-each select="headers">
				<xsl:if test="position()=1">
					...
					"Header" : "Сводный отчет по данным <xsl:choose>
						<xsl:when test="//fn/@val=0">неконсолидированной</xsl:when>
						<xsl:when test="//fn/@val=1">консолидированной</xsl:when>
					</xsl:choose> бухгалтерской отчетности, <xsl:for-each select="//currency">
						<xsl:if test="@sel=1">
							<xsl:value-of select ="@name"/>
						</xsl:if>
					</xsl:for-each>",
					...
					{"Style" : "TabHeader","colspan":1, "rowspan":1, Type:"S", "Data" : "Код строки"},
					<xsl:for-each select="LA/dta">
						{"Style" : "TabHeader","colspan":1, "rowspan":1, Type:"S", "Data" : "<xsl:value-of select="concat(@y,'г.')"/>"},
					</xsl:for-each>]},
				</xsl:if>
				<xsl:if test="//cur and position()=1">
					...
					{Row :[{"Style" : "even","colspan":1, "rowspan":1, Type:"S", "Data" : "Курс на дату"},{"Style" : "even","colspan":1, "rowspan":1, Type:"S", "Data" : ""},<xsl:for-each select="//cur">
						{"Style" : "even","colspan":1, "rowspan":1, Type:"D", "Data" : "<xsl:value-of select="@cd"/>"},
					</xsl:for-each>]},
				</xsl:if>
				{Row :[{"Style" : "<xsl:choose>
					<xsl:when test="contains(@name,'font')">subcaption</xsl:when>
					<xsl:otherwise>razdel</xsl:otherwise>
				</xsl:choose>","colspan":<xsl:value-of select="//nc/@val+1"/>, "rowspan":1, "Type" : "S", "Data" : "<xsl:value-of  disable-output-escaping="yes" select ="js:replace(string(@name),'&lt;font style=&quot;color: #0066CC;font-size: 15px;font-weight: 700;&quot;&gt;','')"/>"},]},
