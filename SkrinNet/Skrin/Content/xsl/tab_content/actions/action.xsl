				...
				<td style='width:100%; height:inherit' colspan='2'>
					<div class='act_text' id='act_text'>
						...
						<xsl:for-each select="issues">
                            <div class="minicaption" style="margin-top:10px">Государственный регистрационный номер   <xsl:value-of select="@reg_no"/></div>
															        ...
															        <td style="text-align:right;">
																        <xsl:choose>
																	        ...
																	        <xsl:otherwise>
																		        ...
																		        <xsl:choose>
																			        <xsl:when test="contains(@doc_name,'.')">
																				        <xsl:value-of select="format-number(substring-before(@doc_name,'.'),'# ##0','buh')"/>.<xsl:value-of select="substring-after(@doc_name,'.')"/>
																			        </xsl:when>
																			        <xsl:otherwise>
