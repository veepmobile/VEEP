...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
  ...
  <xsl:template match="profile">
    ...
    <xsl:if test="chairman[position()=last()]">
      <span class="subcaption">Председатель совета директоров (наблюдательного совета)</span>
      <table width="98%" cellpadding="0" cellspacing="0">
        <tr>
          <td class="table_caption" style="width:250px;">Ф.И.О</td>
          <td class="table_caption" style="width:75px;">Год рождения</td>
            ...
            <td>
              <div onclick="ec(this)">
                <xsl:attribute name="style">
                  <xsl:if test="//@PDF=-1">display:block</xsl:if>
    ...
    <xsl:if test="members[position()=last()]">
      <span class="subcaption">Члены совета директоров (наблюдательного совета)</span>
      <table width="98%" cellpadding="0" cellspacing="0">
        <tr>
          <td class="table_caption" style="width:250px;">Ф.И.О</td>
          <td class="table_caption" style="width:75px;">Год рождения</td>
        ...
        <xsl:if test="//@PDF=-1">
          <tr>
            <td colspan="4" style="text-align:right;">
              <span val="0" onclick="ec();" style="cursor:pointer;color:#003399;" id="bexpand">
                Показать должности
              </span>
            ...
            <td>
              <xsl:if test="string-length(@bio) &gt; 0">
                <div onclick="ec(this)">
                  ...
                  <xsl:attribute name="id">col<xsl:value-of select="position()"/></xsl:attribute>
                  <xsl:value-of disable-output-escaping="yes" select="substring(@bio,0,20)"/>...<span style="cursor:pointer;color:#003399;">Подробнее</span><img src="/images/tra_e.png" alt="" style="padding-left:3px"/>
                </div>
                <div onclick="ec(this)">
                  <xsl:attribute name="id">exp<xsl:value-of select="position()"/></xsl:attribute>
                  <xsl:attribute name="style">
                    <xsl:if test="//@PDF=0">display:block</xsl:if>
                    <xsl:if test="//@PDF=-1">display:none</xsl:if>
                  </xsl:attribute>
    ...
    <script language="javascript" type="text/javascript">
      <![CDATA[ 
			xls_params={"period": $("#per").val(),"iss":$("#iss").val(),"module" : "sovdir/","x":Math.random()}
      ...
      function goPeriod(period){
        showClock();
        $("#tab_content").load("/Tab/Index/?id=21&ticker=]]><xsl:value-of select="//@iss"/><![CDATA[&period=" + period, 
            function(data){
                hideClock();
            }
        );
    ...
    <input id="tabId" type="hidden" value="21" />
  </xsl:template>
</xsl:stylesheet>
