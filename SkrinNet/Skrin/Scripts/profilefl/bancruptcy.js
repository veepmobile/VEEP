...
(function () {
    ...
    window.bc_search = function (page) {
        showClock();
        bcSO.page = page;
        var url = "/ProfileFL/BancruptcySearch";
        var params = bcSO.toString();
        $.ajax({
            url: url,
            type: "POST",
            data: params,
    ...
    window.bc_save_search_params = function () {
        ...
        if ($('input[name="vkl"]:checked').val() == 2)
            bcSO.iss = inn;
        else
            bcSO.iss = fio;
    ...
    var bc_get_date = function () {
        var url = "/ProfileFL/BancruptcyGetMessageDates";
        var params = bcSO.toString();
        $.ajax({
            url: url,
            type: "POST",
    ...
    var load_type_selectors = function () {
        if (!mes_types) {
            showClock();
            bc_save_search_params();
            var url = "/ProfileFL/BancruptcyGetMessageTypes";
            var params = bcSO.toString();
            $.ajax({
                url: url,
                type: "POST",
    ...
    var showBankrotSelected = function (all) {
        if (all) {
            var ids_ar = [];
            var params = bcSO.toString();
            $.post('/ProfileFL/BancruptcyGetAllIds', params, function (data) {
