...
(function () {
    ...
    set_xls_function(function () {
        ...
        form.action = "/Zakupki/ZakupkiDoSearchExcel";
        form.method = "POST";
   ...
   /* var init_calendar= function() {
    ...
    function load_223_gi(gi) {
        if (gi.reg_num != null)
            $('#contract_header').text("Контракт № ФЗ 223 " + gi.reg_num);
    ...
    function load_data44(CONTRACT_ID, finish_fn) {
        ...
        $.post("/Zakupki/GetCont44Data/?"+ par, function (data) {
            ...
            load_44_gi(data.gi, data.source_id);
            load_44_ci(data.ci);
            load_44_gd(data.gd);
            load_bud(data.bud, 1);
    ...
    function load_44_gi(gi,src) {
        if (gi.reg_num != null)
           
            $('#contract_header').text("Контракт № ФЗ " + ((src=="3")?"44":"94") + " " + gi.reg_num);
