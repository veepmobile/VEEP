...
function showeventslist(pg) {
    ...
    if (search == "1") {
        ...
        params = {
            isSearch: 1, DBeg: Date2Str(Str2Date(getValue("DBeg"), 0, 0, 0), 1), DEnd: Date2Str(Str2Date(getValue("DEnd"), 0, 0, 0), 1), pg: pg, KW: getValue("KW"),
...
function shownewslist(pg) {
    ...
    if (search == "1") {
        ...
        params = {
            isSearch: 1, DBeg: Date2Str(Str2Date(getValue("DBeg"), 0, 0, 0), 1), DEnd: Date2Str(Str2Date(getValue("DEnd"), 0, 0, 0), 1), pg: pg, KW: getValue("KW"), RTS_CODE: getValue("news_code"),
...
function showClock() {
    ...
    d.innerHTML = '<table width="100%" height="100%"><tr valign="middle"><td align="center"><img src="/Content/img/wait.gif" align="absmiddle"/></td></tr></table>';
...
function Date2Str(Data, format) {
    ...
    switch (format) {
        case 0: {
            retStr = PadL(Data.getDate(), "0", 2) + "-" + PadL(Data.getMonth() + 1, "0", 2) + "-" + Data.getFullYear();
            break;
        }
        case 1: {
            retStr = Data.getFullYear() + "-" + PadL(Data.getMonth() + 1, "0", 2) + "-" + PadL(Data.getDate(), "0", 2);
            break;
        }
...
function getReport(ogrn, inn, rep_ready) {
    ...
    if (rep_ready == 1) {
        ...
        txtOgrn = ogrn;
        txtInn = inn;
        showwin('info', '<span id=\"lblError\" style=\"color:#000;\">Формируется Выписка из ЕГРЮЛ, пожалуйста, подождите</span><br /><div id=\"rotor\" style=\"text-align:center;padding-top:20px;\"><img src="/Content/img/wait.gif" align="absmiddle"/></div>', 0);
...
function waitMassReg() {
    showwin('info', '<span id=\"lblError\" style=\"color:#000;\">Осуществляется поиск адресов массовой регистрации, пожалуйста, подождите</span><br /><div id=\"rotor\" style=\"text-align:center;padding-top:20px;\"><img src="/Content/img/wait.gif" align="absmiddle"/></div>', 0);
}
...
function doPrint(id, title) {
    $("#" + id).printElement({
        pageTitle: title + ".html", leaveOpen: false,
        printMode: 'popup'
    });
}
